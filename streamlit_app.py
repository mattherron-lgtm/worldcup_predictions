...
def load_fixtures(_client):
    return run_query(_client, f"""
        SELECT
            ...
            c.best_odds_home, c.best_odds_draw, c.best_odds_away,
...
def page_model_performance(client):
    ...
    if len(completed) > 0:
        ...
        # Round goal columns
        for col in ["Pred 1H", "Pred 2H", "Actual 1H", "Actual 2H"]:
...
def page_chat(client):
    ...
    gemini = make_gemini_model(genai, system_instruction=(
        ...
        "Your job is to help the user with any football betting or analysis question, including:\n"
        "- Single bets and identifying value where model probability exceeds bookmaker implied probability.\n"
        "- Bet builders (same-game multis): combine selections within a single match — e.g. match result + both teams to score + player to score. Use xG, win probabilities and form data to reason about each leg.\n"
        "- Accumulators: combine selections across multiple matches. Flag where compounding bookmaker margins erode value, but still help build them if asked.\n"
        "- Stake sizing: use the Fractional Kelly Criterion (0.25x) for value bets; recommend flat small stakes for higher-variance bet builders.\n"
        "- General World Cup analysis: group predictions, knockout bracket, team form, head-to-head history.\n\n"
        "Be helpful, practical and direct. Use the prediction data to reason about each leg of any bet builder or accumulator. "
        "Flag genuine risks but do not refuse requests. Keep responses under 250 words unless a detailed breakdown is asked for.\n\n"
